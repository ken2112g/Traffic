...
export class TikTokWorker extends BaseWorker {
  ...
  async _isLoggedIn() {
    try {
      await this.page.goto('https://www.tiktok.com/', { waitUntil: 'domcontentloaded', timeout: 15000 });
      await this.randomDelay(800, 1500);
      // Nếu có nút Login trên trang chủ => chưa đăng nhập
      const loginBtn = await this.page.$('[data-e2e="login-button"], a[href*="/login"]');
      return !loginBtn;
    } catch { return false; }
  }
  // ─── Login ────────────────────────────────────────────────────────────────
  ...
  async _commentVideo(profileUrl) {
    ...
    const text = _pickComment('tiktok');
